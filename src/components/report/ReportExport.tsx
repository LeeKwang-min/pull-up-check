...
export function ReportExport({ targetRef }: ReportExportProps) {
  const handleSaveImage = useCallback(async () => {
    ...
    const canvas = await html2canvas(targetRef.current, { backgroundColor: '#1C1917', scale: 2 });
  ...
  return (
    <div className="flex gap-3">
      <button onClick={handleSaveImage} className="flex-1 bg-stone-800 hover:bg-stone-700 text-stone-300 font-semibold py-3 rounded-xl text-sm transition-colors border border-stone-700 cursor-pointer">
        이미지 저장
      </button>
