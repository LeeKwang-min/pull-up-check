...
export function BodyDiagram({ asymmetryDetails }: BodyDiagramProps) {
  ...
  return (
    <div className="surface-card rounded-2xl p-5">
      <h3 className="section-label mb-4">신체 균형 분석</h3>
      ...
      <div className="flex justify-center mb-5">
        <svg viewBox="0 0 160 220" className="w-40 h-auto">
          ...
          {/* Base silhouette */}
          <ellipse cx="80" cy="22" rx="14" ry="17" fill="#D6D3D1" />
          <rect x="73" y="38" width="14" height="10" rx="4" fill="#D6D3D1" />
          <rect x="56" y="47" width="48" height="62" rx="8" fill="#D6D3D1" />
          <rect x="36" y="50" width="18" height="38" rx="9" fill="#D6D3D1" />
          <rect x="106" y="50" width="18" height="38" rx="9" fill="#D6D3D1" />
          <rect x="34" y="90" width="16" height="36" rx="8" fill="#D6D3D1" />
          <rect x="110" y="90" width="16" height="36" rx="8" fill="#D6D3D1" />
          <rect x="58" y="112" width="20" height="46" rx="8" fill="#D6D3D1" />
          <rect x="82" y="112" width="20" height="46" rx="8" fill="#D6D3D1" />
          <rect x="60" y="160" width="16" height="40" rx="7" fill="#D6D3D1" />
          <rect x="84" y="160" width="16" height="40" rx="7" fill="#D6D3D1" />
      ...
      <div className="flex items-center justify-center gap-5 mb-6">
        {[
          { color: '#22c55e', label: '균형' },
          { color: '#eab308', label: '약간 차이' },
      ...
      {asymmetryDetails && (
        <div className="space-y-1">
          <h4 className="section-label mb-2">감점 상세</h4>
              ...
              <div key={item.label} className="py-2.5">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[13px] text-stone-600">{item.label}</span>
                    <span className="text-[11px] text-stone-400">{item.weight}</span>
                  </div>
                  <div className="flex items-center gap-2.5 text-[12px]">
                    ...
                    <span className={`font-semibold tabular-nums font-[Barlow_Condensed] ${
                      item.deduction > 5 ? 'text-red-500' : item.deduction > 1 ? 'text-amber-500' : 'text-stone-400'
                    }`}>
                ...
                {item.biasType === 'height' && Math.abs(item.bias) >= 0.3 && (
                  <div className="flex items-center gap-1.5 mt-1 ml-3.5">
                    <span className={`text-[10px] font-mono font-bold ${item.bias > 0 ? 'text-blue-500/70' : 'text-orange-500/70'}`}>
                      {biasArrow(item.bias)}
                    </span>
          ...
          {/* Total */}
          <div className="pt-3 mt-1 border-t border-stone-200 flex justify-between items-center">
            <span className="text-[13px] text-stone-500">합계</span>
            <span className={`text-sm font-bold tabular-nums font-[Barlow_Condensed] ${
              totalDeduction > 20 ? 'text-red-500' : totalDeduction > 5 ? 'text-amber-500' : 'text-emerald-500'
            }`}>
