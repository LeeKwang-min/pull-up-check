...
export function SetChart({ sets }: SetChartProps) {
  ...
  useEffect(() => {
    ...
    const x = d3.scaleBand().domain(sets.map((_, i) => `Set ${i + 1}`)).range([0, width]).padding(0.3);
  ...
  return (
    <div className="bg-zinc-900 rounded-xl p-4">
      <h3 className="text-sm font-semibold text-zinc-300 mb-2">Set-by-Set Analysis</h3>
      <div className="flex gap-4 text-xs text-zinc-500 mb-2">
        <span className="flex items-center gap-1"><span className="w-3 h-3 bg-blue-500 rounded-sm inline-block" /> Reps</span>
        <span className="flex items-center gap-1"><span className="w-3 h-3 bg-amber-500 rounded-full inline-block" /> Form Score</span>
      </div>
