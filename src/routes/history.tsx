  ...
  if (sessions.length === 0) {
    return (
      <div className="py-20 text-center">
        <svg className="mx-auto mb-4" width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="#44403C" strokeWidth="1.5"><path d="M12 8v4l3 3"/><circle cx="12" cy="12" r="10"/></svg>
        <p className="text-stone-500">아직 기록이 없습니다</p>
  ...
  return (
    <div className="py-6 space-y-3">
      ...
      {sessions.map((session) => (
        <Link key={session.id} to="/result/$id" params={{ id: session.id }} className="block bg-stone-900 hover:bg-stone-800 rounded-2xl p-4 transition-colors border border-stone-800 cursor-pointer">
          <div className="flex justify-between items-start">
            <div>
              <div className="font-semibold text-sm">{session.totalReps}회 · {session.sets.length}세트</div>
              <div className="text-xs text-stone-500 mt-0.5">{new Date(session.date).toLocaleDateString()} · {session.angle === 'front' ? '정면' : session.angle === 'back' ? '후면' : '측면'} · {session.inputMode === 'camera' ? '카메라' : '업로드'}</div>
            </div>
