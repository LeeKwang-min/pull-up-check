  ...
  return (
    <div className="py-8 space-y-8">
      <div>
        ...
        <p className="text-stone-400 text-sm mt-1">촬영 각도와 입력 방식을 선택하세요</p>
      </div>

      <div className="space-y-3">
        <label className="text-sm font-medium text-stone-300 uppercase tracking-wider text-xs font-[Barlow_Condensed]">촬영 각도</label>
        <AngleSelector value={angle} onChange={setAngle} />
      </div>

      <div className="space-y-3">
        <label className="text-sm font-medium text-stone-300 uppercase tracking-wider text-xs font-[Barlow_Condensed]">입력 방식</label>
        <div className="grid grid-cols-2 gap-3">
          <Link
            to="/analyze/camera"
            search={{ angle }}
            className="bg-stone-800 hover:bg-stone-700 rounded-2xl p-5 text-center transition-colors border border-stone-700 cursor-pointer"
          >
            ...
            <div className="text-xs text-stone-400 mt-0.5">실시간 측정</div>
          </Link>
          <Link
            to="/analyze/upload"
            search={{ angle }}
          ...
          >
            ...
            <div className="text-xs text-stone-400 mt-0.5">녹화 영상 분석</div>
          </Link>
        </div>
      </div>
    </div>
